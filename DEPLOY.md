...
| Workflow | 触发时间 | 功能 |
|---|---|---|
| `daily-push.yml` | 每天北京时间 08:00 刷新题目；21:00 提醒 | 始终保持当天3个独立待办 |
...
- 每天推送前自动检测昨日待办完成状态
- **未完成的题目** → 删除昨日待办后，以新的当天待办继续
- **已完成或跳过的题目** → 不再续做，由新题补位
- 没有完成或跳过反馈时，题目继续到下一天
- 可在飞书待办点「完成」，或在群内发送「完成第 2 题」
- 在群内发送「跳过第 2 题」后，第二天清理该题并补充新题
- 飞书状态查询或内部状态读取失败时，保留现有任务并停止刷新
